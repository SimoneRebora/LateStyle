###Kafka experiment
###Note1: the network analysis graph should be generated by using Gephi (https://gephi.org/)
###The following code produces a series of csv files ("Kafka_internal_Gephi.csv" et al.) to be processed with Gephi
...
Kafka_young <- cut_short(Kafka_young)
Kafka_middle <- cut_short(Kafka_middle)
Kafka_old <- cut_short(Kafka_old)
...
range_young=c(1907, 1913)
range_mean=c(1914, 1919)
range_old=c(1920, 1926)
...
Stylo_corpus <- c(Kafka_young, Kafka_middle, Kafka_old)
...
file.rename("Kafka_CA_2000_MFWs_Culled_0__dist.cosine.delta_EDGES.csv", "Kafka_internal_Gephi.csv")
...
##Young
Stylo_corpus <- c(Kafka_young, other_authors_young)
...
###rename output csv file for analysis with Gephi
file.rename("Kafka_CA_2000_MFWs_Culled_0__dist.cosine.delta_EDGES.csv", "Kafka_external_young_Gephi.csv")

##Middle
Stylo_corpus <- c(Kafka_middle, other_authors_middle)
results.stylo <- stylo(gui = FALSE, corpus.lang="German", analysis.type="CA", mfw.min=2000, mfw.max=2000, mfw.incr=0, distance.measure="dist.cosine.delta", write.pdf.file = FALSE, parsed.corpus = Stylo_corpus)
###rename output csv file for analysis with Gephi
file.rename("Kafka_CA_2000_MFWs_Culled_0__dist.cosine.delta_EDGES.csv", "Kafka_external_middle_Gephi.csv")

##Old
Stylo_corpus <- c(Kafka_old, other_authors_old)
results.stylo <- stylo(gui = FALSE, corpus.lang="German", analysis.type="CA", mfw.min=2000, mfw.max=2000, mfw.incr=0, distance.measure="dist.cosine.delta", write.pdf.file = FALSE, parsed.corpus = Stylo_corpus)
###rename output csv file for analysis with Gephi
file.rename("Kafka_CA_2000_MFWs_Culled_0__dist.cosine.delta_EDGES.csv", "Kafka_external_old_Gephi.csv")
...
##Just external...
png("Kafka_oppose_ext_old.png", width = 2000, height = 2000, res = 300)
oppose(gui = FALSE, primary.corpus = Kafka_old, secondary.corpus = other_authors_old, text.slice.length=3000, text.slice.overlap=0, oppose.method="craig.zeta", visualization="markers")
dev.off()

png("Kafka_oppose_ext_middle.png", width = 2000, height = 2000, res = 300)
oppose(gui = FALSE, primary.corpus = Kafka_middle, secondary.corpus = other_authors_middle, text.slice.length=3000, text.slice.overlap=0, oppose.method="craig.zeta", visualization="markers")
dev.off()

png("Kafka_oppose_ext_young.png", width = 2000, height = 2000, res = 300)
oppose(gui = FALSE, primary.corpus = Kafka_young, secondary.corpus = other_authors_young, text.slice.length=3000, text.slice.overlap=0, oppose.method="craig.zeta", visualization="markers")
dev.off()